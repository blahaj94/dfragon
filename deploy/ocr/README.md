...
기존 LDB 운영 API의 `passkey.returnUrl`이 `ldb://auth/callback`이면 그대로 유지합니다. OCR 배포를 위해 Desktop 복귀 주소나 설치 앱을 변경하지 않습니다. 현재 API는 이 기존 운영 주소와 새 `dfragon://auth/callback`을 모두 허용하며, 요청별 고정 주소 검증은 유지합니다.
