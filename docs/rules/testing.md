...
변경한 동작이 실패했을 때의 사용자 피해, 실제 영향 범위와 복구 비용에 맞춰 필요한 최소 검증을 선택한다. 관련 요구사항과 중요한 실패를 확인하면 종료하며, 추가 검증마다 무엇을 더 확인하는지 설명할 수 있어야 한다. 별도 검증 계획서나 사례별 승인 절차는 만들지 않는다.

경로 이름만으로 영향이 없다고 판단하지 않고 호출부·소비자와 실행 경계를 확인한다. 공통 설정·의존성·공유 계약·빌드 산출물처럼 영향이 넓거나 불명확하면 관련 소비자로 범위를 넓힌다. 모든 지원 환경이나 가상의 장애 조합을 자동으로 전수 검증하지 않는다. 현재 명령과 포함 관계는 [scripts 안내](../../scripts/README.md#native-validation)와 [Repository Map](../reference/repository-map.md)에서 확인한다.
...
영향이 좁고 실패해도 피해가 작으며 쉽게 복구할 수 있는 변경은 기존 검사와 직접 동작 확인으로 충분하면 신규 자동 테스트를 추가하지 않는다. 예를 들어 정적 문구·배치 변경은 필요한 build·시각 확인으로 마칠 수 있다. 수동 확인을 사용하면 확인한 동작과 결과를 PR에 짧게 남긴다. 이를 위해 테스트 프레임워크나 별도 harness를 선행 구축하지 않는다.

인증·인가·비밀정보, 데이터 무결성·삭제·복구, 과금과 중요한 동시성의 보호 공백은 피해에 맞는 회귀·경계 검증으로 확인한다. 정상 흐름 확인만으로 이를 대신하지 않는다. 명시된 제품별 필수 검증과 기존 CI·required check는 유지하며, 이 기준만으로 미검증 출시 조건을 충족하거나 생략한 것으로 처리하지 않는다.
