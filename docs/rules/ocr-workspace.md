...
개발자 화면의 HUD·파티원창 수집에서 사용자가 Print Screen을 눌러 로컬 크롭 저장을 마치면, 그 키 입력 시점에 앱 로그인이 완료된 경우 같은 프레임의 원본 PNG와 선택한 슬롯 좌표를 한 묶음으로 즉시 전송한다. 주기적 미리보기·일반 실시간 OCR은 전송하지 않는다. 원본 게임 화면이 전송된다는 안내를 수집 화면에 표시한다. 비로그인 캡처를 나중에 로그인했다고 보내거나 기존 로컬 자료를 일괄 올리지 않는다. 로컬 정답·모델 예측을 서버 정답으로 전송하지 않는다.

Main이 원본 픽셀·좌표와 인증 coordinator의 access token을 소유하고 고정 HTTPS `POST /api/desktop/captures`로 요청한다. Renderer는 업로드 URL·계정·토큰·원본을 제출하는 IPC를 갖지 않는다. 이 경로는 Origin이 없는 Bearer 요청만 받고, 큰 JSON을 읽기 전에 기존 인증 API의 `/me`로 활성 세션과 고정 owner를 확인한다. 쿠키 로그인·자료 열람·정답·분할·export 권한으로 확장하지 않는다. 다른 자료실 API는 기존 owner cookie·Origin 경계를 유지한다.
