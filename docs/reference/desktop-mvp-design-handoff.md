...
창 크기를 900·700·500·300px로 바꾸고 하단에서 상태 비교·네 면 비교를 선택한다. 각 카드 본문은 독립적으로 순환하고 입력·서버·상세 버튼은 면을 넘기지 않는다. 미리보기 이름 수정은 API를 호출하지 않으며 이전 상세 열기를 막는다. 실패·검색 중·0건·빈 상태를 따로 표시한다.
