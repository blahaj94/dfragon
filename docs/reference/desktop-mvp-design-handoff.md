...
| 저장소 위치 | 이어받을 책임 |
| --- | --- |
| `apps/desktop/src/frontend/src/features/search/components/SearchResults.tsx`, `features/search/components/CharacterCandidates.tsx` | 기존 슬롯·후보 표시를 카드 화면으로 연결할 출발점 |
| `apps/desktop/src/frontend/src/features/search/hooks/useCharacterSearch.ts` | 이름 수정·제출·OCR 복귀·슬롯별 검색 상태 |
| `apps/desktop/src/frontend/src/features/search/api/capture-search.ts`, `connection.ts` | capture와 요청 수명, 순서·취소·늦은 응답 처리 |
...
`src/frontend/src/app/pages/party/PartyPage.tsx`와 `app/pages/character-detail/CharacterDetailPage.tsx`가 `features/character/components/`를 조합해 메인 카드와 상세 A안을 실제 renderer에서 실행한다. `src/frontend/src/testing/fixtures/mvp/`는 합성 데이터와 로컬 디자인 자산을 제공하는 별도 HTML 진입점이다. 기본 `dev`는 이 미리보기를 열고 소스 수정은 HMR로 반영한다. 기존 검색·인증·캡처 화면은 `pnpm --filter @ldb/desktop dev:app`으로 실행한다.
