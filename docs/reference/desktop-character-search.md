...
| `apps/desktop/src/frontend/src/sections/search/capture-search.ts`, `apps/desktop/src/frontend/src/hooks/useCharacterSearch.ts`           | Start별 수명, 로컬 관측 revision, 즉시 표시 제거와 슬롯별 retry 진행 상태를 연결한다.                                                                                |
