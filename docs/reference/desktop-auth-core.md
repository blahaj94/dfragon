...
Ready 뒤에는 dependency를 만들어 coordinator runtime을 구성한다. 앱 자체 저장소 접근 안내를 기다리는 단계는 없으며, 저장소 조회는 main window·IPC 연결 뒤 `start()`를 호출할 때 시작한다. Dependency·clock·coordinator 생성과 이후 owned composition의 예상 밖 예외는 비인증 fallback으로 축약하지 않고 ingress를 폐기한 뒤 nonzero 종료한다. 정상적인 저장소·network 복원 실패는 `storageBlocked` 또는 `restorePaused` 같은 terminal/retry 상태로 resolve하고, owner가 active인 동안의 예상 밖 `start()` rejection도 같은 fatal 경계를 사용해 영구 `restoring`을 남기지 않는다. `before-quit`과 `will-quit`은 취소 가능한 시도로 취급해 새 activation과 뒤늦은 구성을 잠시 막되 protocol ingress는 폐기하지 않는다. Event, window `close` 또는 renderer `beforeunload`가 종료를 취소하면 active 상태를 복구하고 보류한 owned failure를 다시 처리하며, `quit`이 실제 종료를 확정한 뒤에만 ingress를 폐기한다. Bootstrap 또는 성공한 `start()`가 quit 시도 중 끝나면 결과를 버리지 않는다. Bootstrap composition과 이미 받은 protocol callback/activation은 quit 결과까지 보류하고, 취소 시 한 번 재개하며 확정 종료 시 폐기한다. 확정된 정상 quit 뒤의 늦은 rejection은 종료 상태를 바꾸지 않는다. Bootstrap 대기 중 quit이 시작되면 IPC·window·restore를 뒤늦게 만들지 않는다. Window는 permission·capture/auth IPC·navigation handler와 document load 시작이 모두 구성된 뒤에만 current owner로 공개한다. 동기 구성 실패는 등록한 auth IPC와 미공개 window를 rollback하며, current owned window의 document load rejection은 nonzero 종료한다. 다만 cancelable `close` 시도 중 겹친 rejection은 보류하고 `closed`가 실제 종료를 확정하면 폐기한다. 동기 `close` 취소나 renderer `beforeunload` 취소가 확인되면 같은 rejection을 다시 fatal 경계에서 처리한다. 이미 교체됐거나 확정된 정상 quit 뒤에 도착한 늦은 rejection은 현재 owner를 건드리지 않는다. Auth IPC는 current window·webContents·main frame이 모두 살아 있고 frame이 attached 상태의 exact document이며 sender identity도 일치할 때만 허용한다. Window·auth/capture IPC·activate lifecycle을 먼저 연결한 뒤 `start()`로 restore를 시작하고, protocol return은 start 성공 뒤에만 전달한다.
...
| `apps/desktop/src/backend/auth/runtime-effects.ts`                           | config를 자체 캡처하지 않고 bootstrap이 넘긴 적용 설정으로 auth HTTP, macOS credential store, browser, consumer별 clock·entropy를 구성        |
| `apps/desktop/src/backend/auth/runtime-clock.ts`                             | 고정 offset 기준, 표본 폭과 power event에 따른 clock 신뢰 구간을 관리                                                                              |
| `apps/desktop/src/backend/auth/bootstrap.ts`                                 | 단일 trusted config를 effect dependency factory에 전달하고 coordinator runtime을 노출하며, caller가 UI/IPC 뒤 한 번 `start()`하도록 보장 |
...
제품의 macOS credential adapter는 platform Rule의 safeStorage, atomic replacement, file/directory sync, ownership·symlink·permission 검사를 구현한다. 다만 mock의 `confirmed`와 Node IO 성공은 실제 filesystem의 전원 손실 durability나 native ACL·Keychain evidence가 아니며, 그 보장을 배포 선행 조건으로 삼지 않는다. 실제 보호 검사와 파일 작업의 실패 처리는 유지한다.
...
개발 tuple은 `apps/desktop/build/development-auth.json`에서 관리한다. API는 `https://localhost:3443`, providers는 passkey, 앱 복귀는 `dfragon.dev://auth/callback`, 환경은 development, 앱 ID는 `dfragon.dev`다. 서버 패스키 설정과 같은 origin·복귀 주소를 사용한다. API 시작과 CA 신뢰는 [localhost HTTPS 안내](api-start-development.md#같은-컴퓨터에서-desktop과-api-연결)를 따른다.
...
`electron-builder.development.mjs`는 기본 packaging 설정을 재사용하며 `DFRAGON Development`, `dfragon-dev.exe`와 `dfragon.dev` protocol을 선언한다. NSIS는 top-level `protocols`만으로 Windows handler를 등록하지 않으므로 `build/development-installer.nsh`가 현재 사용자의 `Software\Classes\dfragon.dev`에 quoted executable·`%1` command를 등록한다. 설치 전과 등록 직전에 병합된 registry의 소유 command를 확인해 다른 앱 등록이면 중단하며, 제거할 때도 현재 command가 이 설치 경로와 같은 경우에만 등록을 지운다. 현재 사용자용 NSIS 설치이며 설치 직후 자동 실행과 publish를 하지 않는다. Node options·inspect fuse 차단도 유지한다. 개발 ID는 로컬 패키지용이고 운영 identity·서명을 확정하지 않는다. 실제 설치·등록은 [로컬 개발용 등록 계약](../rules/desktop-auth-platform.md#로컬-개발용-등록값)의 실행 허용, 서버 패스키 설정 일치와 현재 association 소유권 확인 뒤 수행한다. 충돌하거나 소유권이 불분명한 등록은 덮어쓰지 않는다. 설치 파일 생성만으로 이 조건을 확인했다고 보지 않는다.

Windows profile과 credential은 고정 capability 차단 없이 실제 native 검사와 호출 결과로 판단한다. 권한·모듈·파일 작업 오류는 기존 `preparation-failed` 또는 `storageBlocked` 처리를 따른다. 설치 앱의 cold/warm 복귀, 패스키 인증 후 상태 반영과 보호 기능 사용은 실제 앱에서 확인한다. Compile한 main의 환경변수 없는 시작과 native 모듈 부재 처리는 합성 Electron 환경의 결과이며 실제 로그인 성공이 아니다.
