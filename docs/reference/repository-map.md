...
- MVP 카드 미리보기: `pnpm --filter @ldb/desktop mvp:build` 후 `ui:fixture mvp dark`로 합성 메인·상세 카드를 확인한다. 전용 build mode만 미리보기 HTML·데이터·이미지를 포함한다. [디자인 이관](desktop-mvp-design-handoff.md)을 참고한다.
