...
- Character detail HTTP: `GET /characters/:serverId/:characterId`는 Neople의 11개 응답을 최신 JSONB로 저장하고 DB에서 다시 읽어 정제한다. `src/characters/details`와 [캐릭터 상세 계약](../rules/character-details.md), [DBML](character-details.dbml)을 참고한다.
