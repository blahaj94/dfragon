...
| `apps/desktop/src/frontend/src/App.tsx`, `pages/login/LoginPage.tsx`, `sections/LoginSection.tsx`                         | 계정 안내와 별개로 직접 검색·PartyCapture를 표시한다. 인증 로딩·실패·계정 전환 때도 이 content를 유지한다.                                                                                                      |
