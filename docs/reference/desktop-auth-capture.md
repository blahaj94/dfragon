...
| `apps/desktop/src/frontend/src/app/App.tsx`, `app/pages/login/LoginPage.tsx`, `features/auth/components/AuthPresentation.tsx`                                 | 계정 안내와 별개로 직접 검색·PartyCapture를 표시한다. Welcome·인증 로딩·실패·계정 전환 때도 이 content를 유지한다.                                                                                              |
| `apps/desktop/src/frontend/src/app/components/PartyCapture.tsx`                                                                    | 기존 source/interval·Start/Stop·인식값 UI와 네 슬롯 검색 결과를 표시한다. 공용 UI 외형을 변경하지 않는다.                                                                                                       |
| `apps/desktop/src/frontend/src/features/capture/hooks/usePartyCaptureSession.ts`, `usePartyRecognition.ts`, `useCaptureSourceSelection.ts` | 현재 capture의 AbortSignal을 OCR에 전달하고 종료 뒤 결과·통지를 버린다. Unmount에서 stream·video·worker·loop와 main 선택을 정리한다.                                                                            |
...
`main.test.ts`는 인증 설정·provider가 없어도 공개 검색 설정을 연결하고 Windows에서는 capture 수명 검사 결과에 따라 media 요청을 처리하는지 확인한다. `app/App.test.tsx`는 인증 로딩·실패·로그인·로그아웃·auth runId 재연결과 무관한 source 선택 유지, 선택 전 Start 차단과 unmount cleanup을 확인한다. 이 테스트는 Electron/media doubles를 사용하며 실제 설치 앱의 캡처 성공을 대신하지 않는다.

```sh
pnpm --filter @ldb/desktop exec vitest run scripts/auth-capture-fixture src/backend/capture src/backend/main.test.ts src/frontend/src/features/auth src/frontend/src/features/capture src/frontend/src/app/hooks src/frontend/src/app/App.test.tsx src/frontend/src/app/App.capture-controls.test.tsx
