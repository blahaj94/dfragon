...
test('trusted workflow runs default-branch code and isolates the trigger Secret', async () => {
  ...
  assert.match(workflow, /jobs:\s+trigger:/s)
  assert.match(workflow, /conclusion == 'success'/)
  assert.match(workflow, /event == 'pull_request'/)
  assert.match(workflow, /pull_requests\[0\]\.number != null/)
  assert.match(workflow, /run: node scripts\/pr-review\/src\/trigger\.mjs/)
  assert.match(workflow, /persist-credentials: false/)
  assert.doesNotMatch(workflow, /needs:|pull-requests: write|contents: write/)
  assert.doesNotMatch(workflow, /pull_request_target/)
