...
type Win32Api = {
  ...
  MonitorFromPoint: (point: { x: number; y: number }, flags: number) => bigint | null
  GetMonitorInfo: (monitor: bigint, info: Buffer) => number
  EnumDisplaySettings: (device: string, mode: number, settings: Buffer) => number
  CreateDC: (driver: string, device: string, port: null, mode: null) => bigint | null
...
export function capturePrimaryFrame(): PixelFrame {
  ...
  if (!previousDpi) {
    throw fail('SetThreadDpiAwarenessContext')
  }
  ...
  try {
    ...
    if (!api.GetMonitorInfo(monitor, monitorInfo)) {
      throw fail('GetMonitorInfoW')
    }
    ...
    if (
      ...
      throw new Error('Primary monitor exceeds the developer capture image limits.')
    }
    ...
    if (memoryDC && previousBitmap) {
      ...
      const currentBitmap = previousBitmap
