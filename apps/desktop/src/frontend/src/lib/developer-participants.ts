...
export function getParticipantPreviewMessage(code: string): { title: string; detail: string } {
  if (code === 'DEVELOPER_GAME_NOT_FOUND') {
    return {
      title: '던전앤파이터를 실행해주세요.',
      detail: '게임 창을 찾으면 미리보기를 시작해요.'
    }
  }
  if (code === 'DEVELOPER_PARTICIPANT_WINDOW_NOT_FOUND') {
    return { title: '파티참가인원 창을 열어주세요.', detail: '창이 보이면 자동으로 다시 찾을게요.' }
  }
  if (code === 'DEVELOPER_PARTY_SLOTS_NOT_FOUND') {
    return {
      title: '저장할 닉네임이 없어요.',
  ...
  if (code === 'DEVELOPER_STORAGE_UNAVAILABLE' || code === 'DEVELOPER_OPERATION_FAILED') {
    return {
