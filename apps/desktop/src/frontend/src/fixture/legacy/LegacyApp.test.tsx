...
it('auth 로딩과 연결 실패 중에도 직접 검색과 게임 창 선택을 사용할 수 있다', async () => {
  ...
  expect(container.querySelector<HTMLButtonElement>('[aria-label="로그인"]')?.disabled).toBe(true)
  ...
  expect(container.querySelector<HTMLButtonElement>('[aria-label="로그인"]')?.disabled).toBe(false)
  expect(container.querySelector('form button')?.hasAttribute('disabled')).toBe(false)
