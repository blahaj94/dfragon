...
- Frontend `src/frontend/src/components`는 하위 폴더 없이 배치하고 파일당 컴포넌트 하나를 선언한다. 컴포넌트의 StyleX 정의는 옆의 `{name}.style.ts`에 분리하고, 컴포넌트에서는 named export를 가져와 적용한다. 전용 스타일이 없는 컴포넌트에 빈 스타일 파일을 만들지 않는다.
