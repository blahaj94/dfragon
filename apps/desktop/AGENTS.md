...
- Frontend 공통 유틸리티는 `src/frontend/src/lib`, 공통 타입은 `types`, 상수는 `constants`에서 역할별 파일로 관리하고 직접 import한다. 이 모듈은 UI 계층이나 fixture를 import하지 않는다. 요청·구독 수명을 관리하는 hook·class와 구현 전용 타입은 소유한 기능에 유지하며, process 간 IPC contract는 기존 preload 위치를 사용한다.
