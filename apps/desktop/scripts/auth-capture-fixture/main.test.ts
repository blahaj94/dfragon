...
beforeEach(() => {
  ...
  vi.stubEnv('DFRAGON_AUTH_CAPTURE_PROFILE', undefined)
  vi.stubEnv('DFRAGON_AUTH_CAPTURE_LAUNCHER_PID', undefined)
})
afterEach(() => {
  vi.restoreAllMocks()
  vi.unstubAllEnvs()
...
it('launcher가 지정한 profile만 사용하고 종료 후 삭제는 launcher에 맡긴다', async () => {
  const profile = join(tmpdir(), 'dfragon-auth-capture-fixture-unit01')
  vi.stubEnv('DFRAGON_AUTH_CAPTURE_PROFILE', profile)
  vi.stubEnv('DFRAGON_AUTH_CAPTURE_LAUNCHER_PID', String(process.ppid))
  await import('./main')
