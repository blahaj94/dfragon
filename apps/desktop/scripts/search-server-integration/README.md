...
Desktop `createSearchHttp`가 실제 `apps/api/dist/main.js`와 연결되는지 확인한다. 기존 API 도구로 승인된 PostgreSQL image를 검증하고 고유 container·volume을 생성해 migration을 적용한다. Neople은 loopback fixture이며 공개 검색과 peer IP quota는 실제 API를 사용한다. 로그인·exchange 없이 시작하고 검색 전후 session이 없는지 확인한다. API 시작에 필요한 기존 synthetic 인증 설정과 DB 초기화는 유지한다.
