...
이 간격이 모든 프레임 장식에서 유지된다는 뜻은 아닙니다. [추가 FHD 관측](../../docs/reference/desktop-party-geometry.md#관측과-추정)에서는 같은 화면의 인접 간격이 171px과 181px로 다른 정상 네 프레임도 확인했습니다. 따라서 배율을 간격 하나로 확정하지 말고 프레임의 세로 구조와 다른 앵커로 검증해야 합니다.
...
| `search-limit` | 조건을 만족하는 빨간 연결 영역이 128개를 초과하거나, 프레임 전체 헤더 비교가 64,000,000개 픽셀 표본 한도를 초과하려 합니다. 일부 후보만 검사해 성공을 반환하지 않습니다. |
