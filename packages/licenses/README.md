...
- `notices/lib`: CP949 문자 표의 iconv-lite 고지입니다. `@dfragon/lib` build에서 `dist/notices`로 복사합니다. Desktop이 이 함수를 아직 소비하지 않으므로 Desktop 고지에는 넣지 않습니다.
...
```ts
import { uiNotices, desktopNotices } from '@dfragon/licenses/vite'
```
